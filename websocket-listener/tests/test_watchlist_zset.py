...
def _rdb(**overrides):
    ...
    rdb.smembers = AsyncMock(side_effect=overrides.get("smembers", [set(), set()]))
...
async def test_ranked_candidates_prefers_zset_order():
    ...
    rdb.smembers.assert_not_awaited()
...
async def test_ranked_candidates_falls_back_to_priority_set():
    ...
    rdb = _rdb(zrevrange=[], smembers=[{"000002"}, {"000001", "000002"}])
...
async def test_ranked_candidates_falls_back_to_watchlist_set():
    ...
    rdb = _rdb(zrevrange=[], smembers=[set(), {"000004", "000003"}])
