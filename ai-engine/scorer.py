...
def rule_score(signal: dict, market_ctx: dict) -> tuple[float, dict]:
    ...
    match strategy:
        case "S1_GAP_OPEN":
            ...
            _bid_sc = 0.0
            if s1_bid is not None and s1_bid >= 0:
                _bid_sc = 25 if s1_bid > 2 else (20 if s1_bid > 1.5 else (10 if s1_bid > 1.3 else 0))
            ...
            _sig_sc = min(30, strategy_raw * 0.2)
            score += _gap_sc + _str_sc + _bid_sc + _cntr_sc + _sig_sc
            _momentum_score += _str_sc + _cntr_sc + _sig_sc
            _demand_score   += _bid_sc
            _strategy_data = {"gap_pct": gap, "gap_score": _gap_sc, "strategy_raw": strategy_raw}
        ...
        case "S4_BIG_CANDLE":
            ...
            gain_pct = _safe_float(signal.get("gain_pct", 0))
            _vol_sc  = 30 if vol_ratio > 10 else (25 if vol_ratio > 5 else (15 if vol_ratio >= 3 else 0))
            _body_sc = 20 if body_ratio >= 0.8 else (15 if body_ratio >= 0.7 else (10 if body_ratio >= 0.65 else 0))
            _high_sc = 20 if signal.get("is_new_high") else 0
            _str_sc  = 20 if strength > 150 else (15 if strength > 130 else (10 if strength > 120 else 0))
            _gain_sc = 15 if gain_pct >= 5 else (10 if gain_pct >= 3.5 else (5 if gain_pct >= 2.5 else 0))
            score += _vol_sc + _body_sc + _high_sc + _str_sc + _gain_sc
            _vol_score      += _vol_sc
            _momentum_score += _str_sc + _body_sc + _high_sc + _gain_sc
            _strategy_data = {
                "vol_ratio": vol_ratio,
                "body_ratio": body_ratio,
                "gain_pct": gain_pct,
                "is_new_high": bool(signal.get("is_new_high")),
            }

        case "S5_PROG_FRGN":
            net_amt = _safe_float(signal.get("net_buy_amt", 0))
            s5_strength = _safe_float(signal.get("cntr_strength", 0))
            ...
            if not ws_online_s5 and effective_strength == 100.0:
                _str_sc = 10  # 중립 점수
            else:
                _str_sc = 25 if effective_strength > 130 else (20 if effective_strength > 120 else (10 if effective_strength > 100 else 0))
            _bid_sc = 0.0
            if s5_bid is not None and s5_bid >= 0:
                _bid_sc = 20 if s5_bid > 2 else (15 if s5_bid > 1.5 else (8 if s5_bid > 1.2 else 0))
        ...
        case "S7_ICHIMOKU_BREAKOUT":
            ...
            # 후행스팬 확인 — 추세 견고성 신호
            _chikou_sc = 20 if chikou else 0
            # 거래량
        ...
        case "S10_NEW_HIGH":
            ...
            _sig_sc = min(12, strategy_raw * 0.4)
            score += _vol_sc + _flu_sc + _base_bonus + _str_sc + _bid_sc + _sig_sc
            _vol_score      += _vol_sc
            _momentum_score += _flu_sc + _str_sc + _base_bonus + _sig_sc
            _demand_score   += _bid_sc
        ...
        case "S8_GOLDEN_CROSS":
            ...
            gap_pct = abs(_safe_float(signal.get("gap_pct", 999), 999))
            _cross_sc = 15 if bool(signal.get("is_today_cross", False)) else (10 if gap_pct <= 2 else (5 if gap_pct <= 5 else 0))
            _macd_sc  = 8  if bool(signal.get("is_macd_accel", False)) else 0
            _sig_sc = min(12, strategy_raw * 0.35)
            score += _flu_sc + _vol_sc + _str_sc + _bid_sc + _rsi_sc + _cross_sc + _macd_sc + _sig_sc
            _momentum_score  += _flu_sc + _str_sc
            _vol_score       += _vol_sc
            _technical_score += _rsi_sc + _cross_sc + _macd_sc + _sig_sc
            _demand_score    += _bid_sc
        ...
        case "S13_BOX_BREAKOUT":
            ...
            _vol_sc  = 18 if vol_ratio_s13 >= 3.0 else (12 if vol_ratio_s13 >= 2.0 else (6 if vol_ratio_s13 >= 1.3 else 0))
            _bb_sc   = 10 if bool(signal.get("bollinger_squeeze", False)) else 0
            _mfi_sc  = 8  if bool(signal.get("mfi_confirmed", False)) else 0
            _sig_sc  = min(12, strategy_raw * 0.2)
            score += _flu_sc + _str_sc + _bid_sc + _rsi_sc + _vol_sc + _bb_sc + _mfi_sc + _sig_sc
            _momentum_score  += _flu_sc + _str_sc + _sig_sc
            _vol_score       += _vol_sc
            _demand_score    += _bid_sc
        ...
        case "S14_OVERSOLD_BOUNCE":
            ...
            if bid_ratio is not None:
                _bid_sc = 15 if bid_ratio > 1.5 else (8 if bid_ratio > 1.2 else 0)
            _cond_sc = 18 if cond_cnt >= 3 else (10 if cond_cnt >= 2 else (5 if cond_cnt == 1 else 0))
            _vol_sc = 10 if vol_ratio_s14 >= 1.5 else (5 if vol_ratio_s14 >= 1.1 else 0)
            _sig_sc = min(10, strategy_raw * 0.2)
            score += _rsi_sc + _atr_sc + _str_sc + _bid_sc + _cond_sc + _vol_sc + _sig_sc
            _technical_score += _rsi_sc + _atr_sc + _cond_sc
            _vol_score       += _vol_sc
            _momentum_score  += _str_sc
            _demand_score    += _bid_sc
