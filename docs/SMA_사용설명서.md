...
| 명령어 | 설명 |
|--------|------|
| `/매매중단` | 매매 PAUSE 컨펌 요청 (인라인 키보드) |
| `/매매재개` | 매매 CONTINUE로 즉시 복귀 |
| `/전술 {s1~s7}` | 전술 수동 실행 |
| `/토큰갱신` | 키움 토큰 수동 갱신 |
...
```
□ /상태 → Java API UP, Claude 사용량 확인
□ /에러 → error_queue=0, telegram_queue 정상
□ /뉴스 → 매매 제어 CONTINUE, 시장 심리 확인
□ /이벤트 → 오늘 HIGH 임팩트 이벤트 유무 확인
```
