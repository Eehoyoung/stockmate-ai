...
3. [15가지 매매 전략 상세 설명](#3-15가지-매매-전략-상세-설명)
...
**SMA (StockMate AI)**는 한국 주식 시장에 특화된 자동화 매매 신호 생성 시스템입니다. 키움증권 REST API 및 WebSocket API를 통해 실시간 시세 데이터를 수집하고, 최대 15가지 독립적인 매매 전략을 동시에 실행하여 진입 신호를 탐지합니다. 탐지된 신호는 Claude AI(Anthropic)의 자연어 분석을 거쳐 최종 점수화되며, Telegram을 통해 사용자에게 실시간으로 전달됩니다.
...
- **15가지 전략**: 갭상승·VI 눌림목·기관/외인·장대양봉·프로그램매수·테마·동시호가(데이트레이딩 7개) + 골든크로스·눌림목스윙·신고가돌파·외국인연속·종가강도·박스권돌파·과매도반등·모멘텀동조(스윙 8개)
...
| 전략 다양성 | 제한적 | 단일 조건 위주 | 15가지 복합 전략 |
...
| 스윙 | S8 골든크로스 스윙 | 3~7일 | 09:30~14:30 | ✅ 활성 |
| 스윙 | S9 눌림목 지지 반등 | 3~5일 | 09:30~14:30 | ✅ 활성 |
| 스윙 | S10 52주 신고가 돌파 | 5~10일 | 09:30~14:30 | ✅ 활성 |
| 스윙 | S11 외국인 연속 순매수 | 5~7일 | 09:30~14:30 | ✅ 활성 |
| 스윙 | S12 종가 강도 확인 매수 | 2~5일 | 14:30~15:10 | ✅ 활성 |
| 스윙 | S13 거래량 폭발 박스권 돌파 | 3~7일 | 09:30~14:30 | ✅ 활성 |
| 스윙 | S14 과매도 오실레이터 수렴 반등 | 3~5일 | 09:30~14:00 | ✅ 활성 |
| 스윙 | S15 다중지표 모멘텀 동조 | 5~10일 | 10:00~14:30 | ✅ 활성 |

S1~S7은 Java api-orchestrator가 메인으로 실행합니다. S8~S15는 Python ai-engine의 strategy_runner.py가 실행합니다(`ENABLE_STRATEGY_SCANNER=true` 필요). S8, S9, S13은 HTS 조건검색 의존성이 제거되어 ka10081 일봉 API로 직접 계산합니다.
...
### 3.8 S8 전략: 5일선 골든크로스 스윙

#### 전략 개요

5일 이동평균이 20일 이동평균을 상향 돌파하는 '골든크로스' 당일 또는 직근 3일 이내, 거래량 증가 및 RSI 적정 구간을 확인하여 진입하는 스윙 전략입니다. ka10081 일봉 API로 직접 계산하며 HTS 조건검색이 필요 없습니다.

#### 진입 조건 (AND)

| 조건 | 기준값 |
|------|--------|
| MA5 > MA20 크로스오버 | 당일 또는 직근 3일 이내 (이격 ≤ 5%) |
| 현재가 | ≥ MA60 × 0.95 (60일선 지지권) |
| 당일 거래량 | ≥ 20일 평균 거래량 × 1.3 |
| 당일 등락률 | 0% ~ 15% (양봉 + 과열 제외) |
| RSI(14) | ≤ 75 (이미 과매수인 후행 골든크로스 제거) |
...
### 3.9 S9 전략: 정배열 눌림목 지지 반등 스윙

#### 전략 개요

정배열(MA5 > MA20 > MA60) 상태에서 MA5 근처까지 눌림이 발생한 후 당일 양봉으로 반등 조짐을 보이는 종목을 매수하는 스윙 전략입니다. ka10081 일봉 API로 직접 계산하며 HTS 조건검색이 필요 없습니다.
...
| 현재가 | MA5 기준 -3% ~ +3% (눌림 구간) |
| 당일 양봉 | 현재가 > 시가, 등락률 > 0 |
| 당일 거래량 | ≥ 전일 거래량 × 1.1 (반등 거래량 확인) |
| MA20 이격 | ≤ 15% (추세 내 눌림, 과열 버블 제외) |
| RSI(14) | ≤ 68 (눌림 중 과매수 제거) |
...
| RSI 40~58 (눌림 후 회복 초입, 추가 상승 여력) | +8점 |
...
당일 오후 14:30~15:10에 등락률 4% 이상, 체결강도 110% 이상, 기관 순매수가 확인된 종목을 동시호가 전 매수 검토하는 종가매매 전략입니다. 다음 날 갭 상승 기대를 이용합니다.
...
### 3.13 S13 전략: 거래량 폭발 박스권 돌파 스윙

#### 전략 개요

최근 15거래일 박스권(등락폭 8% 이하)에서 거래량 2배 이상 폭발과 함께 박스 상단을 돌파하는 종목을 매수하는 스윙 전략입니다. ka10081 일봉 API로 직접 계산하며 HTS 조건검색이 필요 없습니다.

#### 진입 조건 (AND)

| 조건 | 기준값 |
|------|--------|
| 박스권 형성 | 최근 15거래일 (고가 - 저가) / 저가 ≤ 8% |
| 박스 상단 돌파 | 오늘 종가 > 15일 최고가 |
| 당일 양봉 | 종가 > 시가 |
| 거래량 폭발 | 당일 ≥ 15일 평균 거래량 × 2.0 |
| 추세 위 돌파 | 현재가 ≥ MA20 |
| 체결강도 | ≥ 120% |
| 당일 등락률 | ≤ 20% (상한가 노이즈 제외) |

#### 보너스 점수

| 조건 | 보너스 |
|------|--------|
| 볼린저 밴드폭(20일) < 6% – 스퀴즈 → 폭발 패턴 | +15점 |
| MFI(14) > 55 – 실제 자금 유입 확인 | +10점 |
...
### 3.14 S14 전략: 과매도 오실레이터 수렴 반등

#### 전략 개요

RSI 과매도(38 이하)에서 복수의 오실레이터(Stochastic, Williams %R, MFI)가 동시에 반등 신호를 보낼 때 진입하는 스윙 전략입니다. 추세 붕괴(-15% 이하)는 제외하며 "일시적 과매도 + 장기 추세 생존 + 반등 신호 2개 이상"을 조건으로 합니다.

#### 필수 조건 (AND)

| 조건 | 기준값 |
|------|--------|
| RSI(14) | 20 ~ 38 (과매도권, 폭락 제외) |
| 현재가 | ≥ MA60 × 0.88 (장기 추세 생존) |
| ATR%(14) | ≤ 4.0% (변동성 정상화 중) |
| 당일 하락폭 | > -5.0% (급락 당일 제외) |
...
| 조건 | 설명 |
|------|------|
| A. Stochastic | %K가 %D를 20 미만 하단에서 상향 돌파 |
| B. Williams %R | -80 상향 돌파 (과매도 탈출 시작) |
| C. MFI | MFI < 30 구간에서 반등 또는 MFI > 25 (세력 매집 유입) |
...
ATR 기반 동적 손절·목표 적용: 손절 ATR × 2.0, 목표 ATR × 3.5 (고정 참고치: 손절 -4%, 목표 +7%)

| 목표 보유기간 | 3~5거래일 | AI 스코어 임계값 | 65점 |
|------------|----------|----------------|------|

---

### 3.15 S15 전략: 다중지표 모멘텀 동조 스윙

#### 전략 개요

4개 모멘텀 지표(MACD, RSI, 볼린저 %B, 거래량)가 동일 방향(상승)을 가리킬 때 진입하는 스윙 전략입니다. 과열(RSI > 70, 볼린저 상단 초과)은 제외하고 "추세 전환 초입~중반, 아직 상승 여력이 있는 구간"을 타겟합니다.
...
| 조건 | 기준값 |
|------|--------|
| A. MACD 모멘텀 | MACD 골든크로스 당일 OR (MACD > 0 AND histogram 2봉 연속 확대) |
| B. RSI 구간 | 48 ~ 68 (추세 시작~중반, 추가 상승 여력) |
| C. 볼린저 %B | 0.45 ~ 0.82 (중심선 위, 상단 미도달) |
| D. 거래량 | ≥ 20일 평균 × 1.3 |
...
ATR 기반 동적 손절: ATR × 2.0 (고정 참고치: 손절 -5%, 목표 +10~15%)

| 목표 보유기간 | 5~10거래일 | AI 스코어 임계값 | 70점 |
|------------|----------|----------------|------|
...
```
□ S12 종가 강도 확인 신호 대기
□ S12 신호 수신 시 15:10 이전 즉시 매수 가능성 검토
```
...
### 11.4 스윙 전략 스캐너 활성화 (S8~S15)

S8, S9, S13은 **ka10081 일봉 API 직접 계산** 방식으로 전환됨. HTS 조건검색 불필요.

1. `ENABLE_STRATEGY_SCANNER=true` 환경변수 설정
2. `strategy_runner.py` 실행 확인
3. 전략별 후보 풀은 CandidateService가 자동 갱신 (스윙: ka10027, 과매도: ka10027, 신고가: ka10016)
...
5. **스윙 전략 스캐너**: S8~S15는 `ENABLE_STRATEGY_SCANNER=true` 설정 시 자동 활성화됩니다.
