...
반면 별도 `formatHoldWatch()` 관심종목 알림은 현재 setup, 현재가, AI/규칙 점수, TP1, SL, RR, 관심 사유 중심이다. family, TP2, 확증 setup, source age, 프로그램 추이까지 모두 표시하는 폼은 아직 아니다. 따라서 “ENTER/HOLD 폼 전체가 완전히 G 전략군형으로 교체됐다”는 설명은 부정확하다.
...
종목: 예시종목 (123456) · 시장: KOSDAQ · 유형: 스윙
확증: S13(primary), S10 / 독립확증 +4 / 상관할인 적용

점수: 규칙 82.0 · AI 78.0 · 표시점수 80.8
상태: hard gate PASS · 데이터 OK · 포트폴리오 PASS
진입: 10,000원 (Kiwoom ask1, age 430ms)
TP1: 10,600원 (+6.0%, 최근 매물대) · 50% 부분청산
TP2: 11,200원 (+12.0%, box 1.0x 확장) · 잔여 추적
SL: 9,650원 (-3.5%, 박스 재진입 무효화선)
RR: raw 1.71 · net/effective 1.55 · 하한 1.55

실행: 체결강도 128 · 호가비 1.73 · spread 0.20% · 추격위험 낮음
...
점수: 규칙 76.0 · AI 81.0 · 표시점수 77.5
보류 사유: 프로그램 순매수는 유지되나 effective RR 1.37 < 1.50

조건부 계획: TP1 10,500 · TP2 11,000 · SL 9,650
필요 개선: ask/spread 정상화, 진입가 9,920 이하 또는 유효 저항 상향
...
- 별도 HOLD_WATCH formatter는 family·TP2·source age·추이 표시가 일반 ENTER보다 제한적이다.
...
권장 후속 구현 순서는 `HOLD_WATCH family 계보 추가 → TP2·method·source age 표시 → 기간이 있는 risk trend 표준화 → formatter 계약 테스트 → Docker canary 재배포`다. 실전 메시지 포맷 변경은 현재 5거래일 성과 canary와 별도 변경으로 추적해야 전후 비교가 흐려지지 않는다.
